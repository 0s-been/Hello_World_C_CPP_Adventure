...
//##추가된 주석은 ##으로 표시했습니다.
//##기존 소스코드는 이 클래스로 모든 이동, 점프, 대시, 애니메이션 업데이트를 처리하는 master클래스였습니다.
//##solid원칙들 srp를 위반하는 것뿐만 아니라
//##결합도가 매우 높아서 유지보수가 어려워 ocp도 위반하는 코드였습니다.


public class PlayerController_before : MonoBehaviour
{
    //##############밑에 변수들을 보면 이동에 필요한 모든 변수들뿐만 아니라
    //##############애니메이션 업데이트 기능들도 이 클래스에 몰려있습니다.
    //기본 이동 관련
    ...
    void Start()
    {
        ...
        //##############이 부분에서 애니메이션과 콤보공격을 직접 참조하므로 결합도가 증가하게됩니다.
    ...
    //##############이 update에서 거의 모든 로직이 처리되고 있습니다.
    //##############srp위반이자 ocp위반입니다.
    void Update()
    {
    ...
    //##############이 부분에서 이동과 점프 시 중력 조절이 이루어지고 있습니다.
    //##############이 또한 playercontroller가 로직을 직접 처리하므로 srp위반입니다.
    void FixedUpdate()
    {
    ...
    //##############playercontroller가 animcontroller를 직접 참조하여 애니메이션 업데이트까지 처리합니다. -> srp위반입니다.
    ...
    //##############이동거리를 계산하는 것까지 playercontroller에서 처리하는 것은 srp위반입니다.
    ...
    private void InputHandle()
    {
        ...
        //############콤보공격까지도 playercontroller가 직접 참조하며 판단하므로 srp위반입니다.
    ...
    //############캐릭터의 이동, 회전을 처리하는 부분하는 부분입니다. srp위반입니다.
    ...
    //############점프 시 실제 물리적인 힘을 가하는 부분입니다. srp위반입니다.
    private void Jump()
    {
    ...
    //############대시를 처리하는 코루틴함수입니다. 이 또한 playercontroller가 직접 대시로직을 처리하므로 srp위반입니다.
    IEnumerator Dash()
    {
        m_isDash = true;
        m_dashcoolTimer = m_dashcool;

        //############대시에서 직접 애니메이션을 처리하는 부분입니다. srp위반입니다.
        //애니메이션 트리거 호출
        if (m_AnimController != null)
        {
